...
// ---------------------------------------------------------------------------
// WS token auth — shared secret between dashboard (Flask) and terminal-server.
// Set TERMINAL_WS_TOKEN in env (generated by entrypoint on first boot, same
// as EVONEXUS_SECRET_KEY derivation pattern). Connections without a valid
// ?token=<value> query param are rejected at the WS upgrade step.
// ---------------------------------------------------------------------------
const _WS_TOKEN = (process.env.TERMINAL_WS_TOKEN || '').trim();

/**
 * Constant-time comparison of two strings to prevent timing attacks.
 * Returns true only when both token and expected are non-empty and equal.
 */
function _verifyWsToken(provided) {
  if (!_WS_TOKEN) {
    // No token configured — running in dev mode without auth enforcement.
    // Log a warning so operators notice.
    console.warn('[security] TERMINAL_WS_TOKEN not set — WebSocket auth disabled');
    return true;
  }
  if (!provided) return false;
  try {
    // Use constant-time comparison; pad to same length before comparing.
    const a = Buffer.from(provided.padEnd(128), 'utf8');
    const b = Buffer.from(_WS_TOKEN.padEnd(128), 'utf8');
...
class TerminalServer {
  ...
  setupExpress() {
    // CORS: restrict to the dashboard origin when EVONEXUS_DASHBOARD_ORIGIN
    // is set (production). Falls back to wide-open for local dev.
    const dashboardOrigin = (process.env.EVONEXUS_DASHBOARD_ORIGIN || '').trim();
    this.app.use(cors(dashboardOrigin ? { origin: dashboardOrigin, credentials: true } : {}));
  ...
  async start() {
    ...
    // verifyClient runs during the HTTP→WebSocket upgrade, before the
    // connection is accepted. Reject any upgrade without a valid token.
    this.wss = new WebSocket.Server({
      server,
      verifyClient: ({ req }, cb) => {
        const url = new URL(req.url, 'ws://localhost');
        const token = url.searchParams.get('token');
        if (_verifyWsToken(token)) {
          cb(true);
        } else {
          // 401 Unauthorized — browser sees this as a failed WS handshake
    ...
    // Bind to loopback only — external callers must go through the Flask
    // proxy (which enforces session auth) or the Traefik forwardauth chain.
    const bindHost = process.env.TERMINAL_BIND_HOST || '127.0.0.1';
    return new Promise((resolve, reject) => {
      server.listen(this.port, bindHost, (err) => {
